...
<body class="page-header-fixed">
  ...
  <div class="row">
                                                             ...
                                                             <?php 
                                                   			...
                                                   			if (mysqli_num_rows($result) > 0) {
                                                   				while($row = mysqli_fetch_assoc($result)) {
                                                   					...
                                                   					$dir_grade = $row["actress_grade"];
                                                   					$dir_status = $row["actress_status"];
                                                   					$dir_rating = $row["actress_rating"];
                                                   					$dir_pic = $row["actress_pic"];
                                                   					...
                                                   					echo "<tr data-id='$dir_id' data-table='actress'>";
                                                   					echo  "<td><img class=\"img-circle avatar\" src=\"$dir_pic\" width=\"40\" height=\"40\"></td>";
                                                   					echo "<td class='cell-name'><a href='actress.php?id=$dir_id' class='btn'>$dir_name</a></td>";
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Add New Actress</h4>
          </div>
	...
	function addNew() {
		...
		$.post('listAjax.php', {action:'add', table:'actress', name:name, rate:rate, grade:grade, status:status, rating:rating}, function(r) {
			if (r.status === 'ok') {
				var cr = (rate / 10000000).toFixed(2);
				var pic = r.pic || '';
				var row = '<tr data-id="'+r.id+'" data-table="actress">';
				row += '<td><img class="img-circle avatar" src="'+pic+'" width="40" height="40"></td>';
				row += '<td class="cell-name"><a href="actress.php?id='+r.id+'" class="btn">'+name+'</a></td>';
				...
				$('#addModal').modal('hide');
				toastr.success('Actress added successfully');
			} else {
				toastr.error(r.msg);
			}
		}, 'json');
	...
	function cancelEdit(btn) {
		var row = $(btn).closest('tr');
		var id = row.data('id');
		row.find('.cell-name').html('<a href="actress.php?id='+id+'" class="btn">'+row.data('origName')+'</a>');
	...
	function saveEdit(btn) {
		...
		$.post('listAjax.php', {action:'update', table:'actress', id:id, name:name, rate:rate, grade:grade, status:status, rating:rating}, function(r) {
			if (r.status === 'ok') {
				var cr = (rate / 10000000).toFixed(2);
				row.find('.cell-name').html('<a href="actress.php?id='+id+'" class="btn">'+name+'</a>');
