...
<body class="page-header-fixed">
  ...
  <div class="row">
                                                             ...
                                                             <?php 
                                                   			...
                                                   			if (mysqli_num_rows($result) > 0) {
                                                   				while($row = mysqli_fetch_assoc($result)) {
                                                   					...
                                                   					$dir_grade = $row["actor_grade"];
                                                   					$dir_status = $row["actor_status"];
                                                   					$dir_rating = $row["actor_rating"];
											                        ...
											                        $dir_pic = '/poster/actors/'.$lower.'.png';
                                                   					...
                                                   					echo "<tr data-id='$dir_id' data-table='actor'>";
                                                   					echo  "<td><img class=\"img-circle avatar\" src=\"$dir_pic\" width=\"40\" height=\"40\"></td>";
                                                   					echo "<td class='cell-name'><a href='actor.php?id=$dir_id' class='btn'>$dir_name</a></td>";
	...
	function addNew() {
		...
		$.post('listAjax.php', {action:'add', table:'actor', name:name, rate:rate, grade:grade, status:status, rating:rating}, function(r) {
			if (r.status === 'ok') {
				var cr = (rate / 10000000).toFixed(2);
				var pic = r.pic || 'poster/' + name.toLowerCase().replace(/\s+/g,'_') + '.png';
				var row = '<tr data-id="'+r.id+'" data-table="actor">';
				row += '<td><img class="img-circle avatar" src="'+pic+'" width="40" height="40"></td>';
				row += '<td class="cell-name"><a href="actor.php?id='+r.id+'" class="btn">'+name+'</a></td>';
	...
	function cancelEdit(btn) {
		var row = $(btn).closest('tr');
		var id = row.data('id');
		row.find('.cell-name').html('<a href="actor.php?id='+id+'" class="btn">'+row.data('origName')+'</a>');
	...
	function saveEdit(btn) {
		...
		$.post('listAjax.php', {action:'update', table:'actor', id:id, name:name, rate:rate, grade:grade, status:status, rating:rating}, function(r) {
			if (r.status === 'ok') {
				var cr = (rate / 10000000).toFixed(2);
				row.find('.cell-name').html('<a href="actor.php?id='+id+'" class="btn">'+name+'</a>');
