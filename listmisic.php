...
<body class="page-header-fixed">
  ...
  <div class="row">
                                                             ...
                                                             <?php 
                                                   			...
                                                   			if (mysqli_num_rows($result) > 0) {
                                                   				while($row = mysqli_fetch_assoc($result)) {
                                                   					...
                                                   					$dir_grade = $row["music_grade"];
                                                   					$dir_status = $row["music_status"];
                                                   					$dir_rating = $row["music_rating"];
                                                   					...
                                                   					echo "<tr data-id='$dir_id' data-table='music'>";
                                                   					echo  "<td><img class=\"img-circle avatar\" src=\"$dir_pic\" width=\"40\" height=\"40\"></td>";
                                                   					echo "<td class='cell-name'><a href='music.php?id=$dir_id' class='btn'>$dir_name</a></td>";
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Add New Music Director</h4>
          </div>
	...
	function addNew() {
		...
		$.post('listAjax.php', {action:'add', table:'music', name:name, rate:rate, grade:grade, status:status, rating:rating}, function(r) {
			if (r.status === 'ok') {
				var cr = (rate / 10000000).toFixed(2);
				var pic = r.pic || '';
				var row = '<tr data-id="'+r.id+'" data-table="music">';
				row += '<td><img class="img-circle avatar" src="'+pic+'" width="40" height="40"></td>';
				row += '<td class="cell-name"><a href="music.php?id='+r.id+'" class="btn">'+name+'</a></td>';
				...
				$('#addModal').modal('hide');
				toastr.success('Music Director added successfully');
			} else {
				toastr.error(r.msg);
			}
		}, 'json');
	...
	function cancelEdit(btn) {
		var row = $(btn).closest('tr');
		var id = row.data('id');
		row.find('.cell-name').html('<a href="music.php?id='+id+'" class="btn">'+row.data('origName')+'</a>');
	...
	function saveEdit(btn) {
		...
		$.post('listAjax.php', {action:'update', table:'music', id:id, name:name, rate:rate, grade:grade, status:status, rating:rating}, function(r) {
			if (r.status === 'ok') {
				var cr = (rate / 10000000).toFixed(2);
				row.find('.cell-name').html('<a href="music.php?id='+id+'" class="btn">'+name+'</a>');
